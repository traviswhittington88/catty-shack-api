...
notificationEvent.on('like', function(db, user_name, meow_id) {
  MeowsService.getById(db, meow_id)
    .then(meow => {
      if (meow && meow.userhandle !== user_name) {
        ...
        MeowsService.createNotification(db, newNotification)
          .then(() => {
  ...
  MeowsService.getById(db, meow_id)
    .then(meow => {
      if (meow && meow.userhandle !== user_name) {
        ...
        return MeowsService.createNotification(db, newNotification);
      }
    })
    .then(() => {
...
notificationEvent.on('delete', function(db, user_name, meow_id) {
  ...
  MeowsService.removeNotification(db, user_name, meow_id)
    .then(() => {
      return;
    })
    .catch(err => {
...
meowsRouter
  ...
  .post(jsonBodyParser, (req, res, next) => {
    ...
    MeowsService.insertMeow(req.app.get('db'), newMeow)
      .then(meow => {
        res
          .status(201)
...
meowsRouter
    ...
    MeowsService.getById(req.app.get('db'), req.params.meow_id)
      .then(meow => {
        if (!meow) {
          ...
          MeowsService.getComments(req.app.get('db'), req.params.meow_id)
            .then(comments => {
              meowData.comments = [];
              comments.forEach(comment => {
    ...
    MeowsService.getById(req.app.get('db'), req.params.meow_id)
      .then(meow => {
        ...
        if (meow.userhandle !== req.user.user_name) {
          ...
          MeowsService.deleteMeow(req.app.get('db'), req.params.meow_id)
            .then(numOfRowsAffected => {
              meowDeleteEvent.emit(
                'delete',
                req.app.get('db'),
                req.params.meow_id,
                req.user.user_name
  ...
  .patch(jsonBodyParser, (req, res, next) => {
    ...
    MeowssService.getById(req.app.get('db'), req.params.meow_id)
      .then(meow => {
  ...
  .post(jsonBodyParser, (req, res, next) => {
    ...
    MeowsService.getById(req.app.get('db'), req.params.meow_id)
      .then(meow => {
        ...
        return MeowsService.incrementCommentCount(
          req.app.get('db'),
          req.params.meow_id,
          meow.commentcount + 1
        );
...
meowsRouter
  ...
  .get((req, res, next) => {
    ...
    MeowsService.getById(req.app.get('db'), req.params.meow_id)
      ...
      .then(likes => {
        if (!likes) {
          return MeowsService.addLike(req.app.get('db'), {
            user_name: req.user.user_name,
            meow_id: req.params.meow_id
          })
            .then(() => {
              meowData.likecount++;
              return MeowsService.incrementLikeCount(
                req.app.get('db'),
                req.params.meow_id,
                meowData.likecount
              );
